...
@Component({
  selector: 'app-guadagni-venditore-chart',
  templateUrl: './guadagni-venditore-chart.component.html',
  styleUrls: ['./guadagni-venditore-chart.component.css', '../guadagni-line-chart/guadagni-line-chart.component.css']
})
export class GuadagniVenditoreChartComponent {
@Input() nomeVenditore!:string;
  ...
  createChart() {
    this.chart = new Chart("guadagniVenditore", {
      type: 'line',
      data: {
  ...
  private updateChart(Gap: string[]) {
    ...
    for(let i=0; i<Gap.length-1; i++){
      observables.push(this.statisticService.getRendimentoVenditore(this.nomeVenditore, Gap[i], Gap[i+1]));
    }
    observables.push(this.statisticService.getRendimentoVenditore(this.nomeVenditore, Gap[Gap.length-1], Gap[Gap.length-1]));
